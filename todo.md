...
- [x] 研究：STP预览功能做成Windows桌面安装软件的可行性（结论：已通过浏览器 Three.js 实现，桌面软件不是必需）
...
- [x] 个人中心右侧添加导航栏（我的文件、收藏零件、个人信息）
- [x] 默认显示"我的文件"tab：存储配额 + 我的文件列表
- [x] 点击"收藏零件"tab：只显示收藏零件内容
- [x] 点击"个人信息"tab：只显示个人信息内容

## 邮件格式在线预览（仅预览，不保存）

- [x] 安装 eml-parse-js 和 dotmsg 依赖
- [x] 添加 .EML 和 .MSG 到支持的文件格式列表
- [x] 实现 EML 文件前端解析与预览组件（发件人、收件人、主题、日期、正文、附件列表）
- [x] 实现 MSG 文件前端解析与预览组件（发件人、收件人、主题、日期、正文、附件列表）
- [x] 邮件预览不提供"保存到我的文件"功能
- [x] 集成到首页文件上传预览流程

## Bug修复：分享链接不支持CSV预览

- [x] ShareView.tsx 添加 CSV 格式支持（SUPPORTED_CSV + CsvViewer 组件）
- [x] 验证 Excel 和 Word 在分享页面的渲染逻辑（已确认代码正确，仅 CSV 缺失）

## 移除加密文件检测限制

- [x] 移除加密文件检测逻辑（不再拦截上传）
- [x] 移除加密文件警告UI和禁用保存按钮
- [x] 允许所有文件正常上传预览（加密文件不提供保存功能由邮件类型逻辑单独控制）

## Markdown 文件预览支持

- [x] 添加 .md 格式到支持的文件类型列表
- [x] 创建 MarkdownViewer 组件（渲染 Markdown 为 HTML）
- [x] 集成到首页文件上传预览流程
- [x] 集成到分享页面预览流程
- [x] Markdown 文件支持保存到我的文件

## 第十七期：压缩包解压下载功能

- [x] 安装 unzipper 和 archiver 依赖库
- [x] 创建后端 API 端点处理压缩包解压
- [x] 自动创建根文件夹结构
- [x] 重新打包成 ZIP 并生成下载链接
- [x] 在压缩包预览界面添加"解压下载"按钮
- [x] 集成前端调用解压 API
- [x] 测试 ZIP 和 RAR 文件的解压功能

## 第十八期：压缩包文件树搜索 + 操作说明更新

- [x] 压缩包预览组件添加搜索框（关键字过滤文件树）
- [x] 搜索时展开所有匹配路径，高亮匹配关键字
- [x] 无匹配结果时显示空状态提示
- [x] 更新操作说明页面，补充新增功能说明（EML/MSG/MD/ZIP/RAR/解压下载）

## BUG修复：RAR 文件列表和解压下载

- [x] RAR 文件上传后显示 0 个文件（文件列表为空）
- [x] RAR 文件解压下载功能不可用（提示不支持）
...
## 前端优化：429 错误友好提示与自动重试

- [x] 分块上传 init 阶段遇到 429 时显示友好提示并自动重试
- [x] 分块上传 chunk 阶段遇到 429 时自动等待- [x] 重试时显示倒计时提示（如“服务器繁忙，5秒后自动重试...”- [x] 429 等待提示卡片中增加“取消上传”按钮，允许用户中断操作�中断操作

## 断点续传支持

- [x] 前端：取消上传时保存 uploadId 和文件指纹到本地缓存
- [x] 前端：再次上传同一文件时检测缓存，恢复 uploadId 并从服务器查询已上传分片
- [x] 后端：取消上传时不立即清除 session，保留一�- [x] 前端：续传时显示“检测到上次未完成的上传，正在继续...”提示��继续..."提示
